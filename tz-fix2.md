...
- Этап 1: CRUD/поиск/пагинация/статистика организаций работают из UI; проверки уникальности и экспорт возвращают корректные ответы.
- Этап 2: Версии документов отображаются, новая версия загружается, удаление версии работает.
- Этап 3: CRUD участников событий и экспорт участников доступны из UI; соблюдается `maxParticipants`.
- Этап 4: Массовое удаление событий и смена статуса работают из UI.
- Этап 5: Прокси-роуты (если включены) корректно транзитят авторизацию и параметры; UI стабилен.
- Этап 6: (опц.) CRUD меню работает (если реализуется сейчас).

...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${ANALYTICS_URL}/inspections/reports${queryString ? `?${queryString}` : ''}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Analytics inspections reports error:', errorText)
      return NextResponse.json({ success: false, message: 'Analytics backend error' }, { status: response.status })
    }
    const data = await response.json()
    return NextResponse.json(data)
  } catch (error) {
    console.error('Error proxying inspections reports:', error)
